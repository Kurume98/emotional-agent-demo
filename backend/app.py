...
# Emotion theme mapping shared with frontend
emotion_themes = {
...
def chat():
    try:
        ...
        if any(word in message_lower for word in ["lonely", "isolated", "alone", "miss", "passed away"]):
            reply = (
                "I hear the weight of loss and loneliness in your words. These feelings are so real, and you're not alone in "
                "experiencing them. Many people in our community share similar experiences. Would you like to talk more about what you're feeling?"
            )
            mood = "sad"

        elif any(word in message_lower for word in ["memory", "forget", "confused", "can't remember"]):
            reply = (
                "Memory changes can be really concerning. It's completely understandable to feel anxious about this. These experiences are more common than you might think, and there are many strategies and support systems available. You're being very brave by sharing this."
            )
            mood = "anxious"

        elif any(word in message_lower for word in ["independence", "driving", "license", "freedom"]):
            reply = (
                "Losing independence, especially something as fundamental as driving, is incredibly difficult. This represents so much more than transportation - it's about autonomy and identity. Your feelings about this are completely valid."
            )
            mood = "sad"

        elif any(word in message_lower for word in ["burden", "family", "children", "worry"]):
            reply = (
                "The fear of being a burden is something so many people share, but let me tell you something important: your family loves you and wants to support you, just as you've supported them throughout your life. This isn't burden - this is love."
            )
            mood = "anxious"

        elif any(word in message_lower for word in ["retirement home", "assisted living", "move"]):
            reply = (
                "Decisions about living arrangements are among the most difficult anyone faces. This isn't just about a place to live - it's about your entire life and identity. Take your time with this decision, and remember that your feelings matter most."
            )
            mood = "anxious"

        elif any(word in message_lower for word in ["aging", "getting old", "time running out"]):
            reply = (
                "These are such profound and universal feelings. Every emotion you're experiencing is completely valid. Your life experience and wisdom are incredible gifts, and every day you have is precious and meaningful."
            )
            mood = "sad"

        elif any(word in message_lower for word in ["technology", "phone", "computer", "confusing"]):
            reply = (
                "Technology can feel overwhelming, especially when it changes so quickly. But your willingness to learn and adapt is remarkable. You're not alone in this - many people share these frustrations, and there are wonderful resources to help."
            )
            mood = "anxious"

        elif any(word in message_lower for word in ["friends gone", "last one", "survived"]):
            reply = (
                "Being the last one remaining is such a profound and lonely experience. The grief of outliving friends and loved ones is real and heavy. Your memories and stories are precious treasures that keep their spirits alive."
            )
            mood = "sad"

        elif any(word in message_lower for word in ["proud", "still active", "exercise", "walk"]):
            reply = (
                "Your pride in staying active is so inspiring! Every step you take is a testament to your resilience and determination. You are showing everyone what aging with dignity and strength looks like."
            )
            mood = "happy"
        ...
        theme = emotion_themes.get(mood, emotion_themes["neutral"])
...
def unified_chat():
    try:
        ...
        message = request_data.get("message", "") or request_data.get("text", "")
        voice_emotion = request_data.get("voiceEmotion")
